...
asv_meta = read_qza("./dada2/FeatureTable[Frequency]_ITS.qza") # Table of metagenome data reads for each ASV by samples
...
tax_meta = read_qza("feature-classifier_classify-sklearn/FeatureData[Taxonomy]_ITS.qza") # Taxonomy table for metagenome data
...
sampledata_meta = read.table(file="./sampledata_its_meta.txt", row.names = 1, header = TRUE, colClasses = "character", sep="\t") # Sample metadata for the metagenome
...
tax_sow = read.table(file="./tax_its_sow.txt", row.names = 1, header = TRUE, sep="\t") # Taxonomy table for cultivation-dependent approach data

sampledata_sow = read.table(file="./sampledata_its_sow.txt", row.names = 1, header = TRUE, colClasses = "character", sep="\t") # Sample metadata for the cultivation-dependent approach data

asv_sow = read.table(file="./asv_its_sow.txt", header = TRUE, sep="\t") # Table of cultivation-dependent approach data reads for each strain by samples
asv_sow = asv_sow %>%
