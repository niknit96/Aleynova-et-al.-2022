...
tax_sow = read.table(file="./Culture-dependent method data/tax_its_sow.txt", row.names = 1, header = TRUE, sep="\t") # Taxonomy table for cultivation-dependent approach data

sampledata_sow = read.table(file="./Culture-dependent method data/sampledata_its_sow.txt", row.names = 1, header = TRUE, colClasses = "character", sep="\t") # Sample metadata for the cultivation-dependent approach data

asv_sow = read.table(file="./Culture-dependent method data/asv_its_sow.txt", header = TRUE, sep="\t") # Table of cultivation-dependent approach data reads for each strain by samples
asv_sow = asv_sow %>%
...
save(physeq, physeq_for_div, V_amur, V_coig, V_vinifera, physeq_for_beta, physeq_for_alfa, physeq_main, sampledata_meta_2, sampledata_sow, physeq_read_counts, physeq_sow, dir,  file = "./ITS/physeq.RData")
