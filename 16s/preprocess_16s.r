...
sampledata_meta = read.table(file="./Metagenome metadata/sampledata_16s_meta.txt", row.names = 1, header = TRUE, colClasses = "character", sep="\t") # Sample metadata for the metagenome
...
tax_sow = read.table(file="./Culture-dependent method data/tax_16s_sow.txt", row.names = 1, header = TRUE, sep="\t") # Taxonomy table for cultivation-dependent approach data

sampledata_sow = read.table(file="./Culture-dependent method data/sampledata_16s_sow.txt", row.names = 1, header = TRUE, colClasses = "character", sep="\t") # Sample metadata for the cultivation-dependent approach data

asv_sow = read.table(file="./Culture-dependent method data/asv_16s_sow.txt", header = TRUE, sep="\t") # Table of cultivation-dependent approach data reads for each strain by samples
asv_sow = asv_sow %>%
...
paste("Taxa of genus level before filtration in V. vinifera-Germany samples:", length(taxa_names(physeq_info_before_filtration_V_Germany))),
...
paste("Taxa of genus level with relative abundance > 0.1% V. vinifera-Germany samples:", length(taxa_names(physeq_info_after_filtration_V_Germany)))
)

write.table(Taxa_of_genus_level, sep = "\t", row.names = FALSE, quote = FALSE, file="./Taxa of genus level (16s).txt")


save(physeq, physeq_for_div, V_amur, V_coig, V_vinifera, physeq_for_beta, physeq_for_alfa, physeq_main, sampledata_meta_2, sampledata_sow, physeq_read_counts, physeq_sow, dir,  file = "./16s/physeq.RData")
